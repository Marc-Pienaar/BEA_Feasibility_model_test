...
def process_results(outputfile):
	'''
	...
	outputfile : String
		Filename of the csv file to process.

	Returns
	-------
	A processed technology csv output
	
	'''	
	#parameters for the scenario - to be supplied by the user - we can make some assumptions for our purposes.
	offroad_v_capacity = 23.6 	#tons        https://sasri.org.za/storage/2019/07/Poster-truck-legal-payload.pdf
	offroad_h_cost = 261.74   	#ZAR/hour
	road_v_capacity = 30.5		#tons
	road_h_cost = 247.95		#ZAR/hour
	load_cost = 14.4			#ZAR/ton
	unload_cost = 12.32			#ZAR/ton
	harvest_cost = 0			#ZAR/ha
	speed_road = 50				#km/hour
	speed_offroad = 8			#km/hour
	#conversion_efficiency =0    #volume of end product/volume of feedstock
	capex = 0                   #user defined Parameter in R/ton of dry biomass or calculated from cost curve data
	opex = 0                    #user defined parameter in R/ton of dry biomass or calculated from cost curve data
	#parameter files
	distances_csv = outputfile
	cost_curves = os.path.sep.join(['parameters', 'Technology cost curves.csv'])
	feedstock_energy = os.path.sep.join(['parameters', 'Feedstock_energy_densities.csv'])
	impact_data = os.path.sep.join(['parameters', 'BioEnergy_impacts.csv'])
	product_energy = os.path.sep.join(['parameters', 'Biofuel energy densities.csv'])
	##read in the route distance and volume table
	trans_df=pd.read_csv(distances_csv, skiprows = 0)
	#calculate some of the needed values for cost estimation
	#Harvest Cost
	trans_df['total_harvest_cost'] = 0    # or  = ha_area * harvest_cost
	#Load Cost
	trans_df['load_cost'] = trans_df['tonnes_contributed'] * load_cost
	#Transport cost
	trans_df['road_trips'] = (trans_df['tonnes_contributed']/road_v_capacity)
	trans_df['road_trip_dist'] = (trans_df['road_trips'] * (trans_df['Arterial_Road_(m)'] + trans_df['Interchange_(m)'] + trans_df['Main_Road_(m)'] + trans_df['National_Freeway_(m)']+trans_df['National_Road_(m)']+trans_df['None_(m)']+trans_df['On/OffRamp_(m)']+trans_df['Other_Road_(m)']+trans_df['Secondary_Road_(m)']+trans_df['Street_(m)']))/1000
	trans_df["travel_time_road"] = trans_df['road_trip_dist']/speed_road
	trans_df['virtual_road_trips'] = (trans_df['tonnes_contributed']/offroad_v_capacity)
	trans_df['virtual_trip_dist'] = (trans_df['virtual_road_trips']* trans_df['Virtual_road_(m)'])/1000
	trans_df["travel_time_virtual"] = (trans_df['virtual_trip_dist']/speed_offroad)
	trans_df['transport_cost'] = (trans_df["travel_time_road"]*road_h_cost) + (trans_df["travel_time_virtual"]*offroad_h_cost)
	#Unload Cost
	trans_df['unload_cost'] = 0 #or = trans_df['tonnes_contributed'] * unload_cost
	
	#Total cost to facility
	trans_df['total_feedstock_cost'] = trans_df['total_harvest_cost']+ trans_df['load_cost'] + trans_df['transport_cost'] + trans_df['unload_cost']
	
	##Calculate a unique Facility ID
	trans_df['capacity'] = trans_df['capacity'].apply(str)
	trans_df['end_point_id'] = trans_df['end_point_id'].apply(str)
	trans_df['unique_id'] = trans_df['technology'] + '_' + trans_df['feedstock']+ '_' + trans_df['capacity']+ '_' + trans_df['end_point_id']
	
	#Pivot the dataframe so that costs are accumulated per facility
	trans_df2 = pd.pivot_table(trans_df, values = ['tonnes_contributed','load_cost','transport_cost','unload_cost','total_feedstock_cost'], index= ['feedstock','technology', 'X_coords', 'Y_coords', 'unique_id', 'end_point_id'], aggfunc=np.sum)
	
	id = trans_df.at[0,'technology']
	
	capacity = trans_df.at[0,'capacity']
	...
	curve = cost_df[cost_df.ID.str.contains(id,regex=False,case=False)]	
	cap_slope = curve['Cap_slope'].iloc[0]
	cap_int = curve['Cap_int'].iloc[0]
	op_slope = pd.to_numeric(curve['Op_slope'].iloc[0])
	op_int = curve['Op_int'].iloc[0]
	conversion_efficiency = (curve['Efficiency'].iloc[0])/100
	output_product = curve['Prod_id'].iloc[0]
	
	#Calculate the Capex, Opex, Conversion Efficiency
	trans_df2['capex'] = (cap_slope*(np.log(trans_df2['tonnes_contributed'])) + cap_int) * trans_df2['tonnes_contributed'] #Capital 
	
	trans_df2['opex'] = op_slope * (np.log(trans_df2['tonnes_contributed'])) + op_int #Operational Expenditure per anum based on the tons contributed
	trans_df2['product_volume_anum'] = trans_df2['tonnes_contributed']*conversion_efficiency #Volume OF bioEnergy product per anum is in tons
	trans_df2['total_production_cost_anum'] = trans_df2['capex'] + trans_df2['opex'] + trans_df2['total_feedstock_cost'] #total cost per anum of feedstock processed
	trans_df2['cost_ton'] = trans_df2['total_production_cost_anum']/ trans_df2['product_volume_anum'] #total cost per ton of feedstock processed
	#Calculate Energy Equivalents
	productE_df=pd.read_csv(product_energy, skiprows = 0)
	Product_Energy_info = productE_df[productE_df.Prod_id.str.match(output_product,case=False)]
	Energy_Density = Product_Energy_info['Energy_(MJ/kg)'].iloc[0]
	trans_df2['energy_generation_Kwh'] = ((trans_df2['product_volume_anum']*1000)*Energy_Density)/3.6 #Energy Equivalent in Kwh
	#Calculate impacts
	tech_impacts = pd.read_csv(impact_data, skiprows = 0)
	impact = tech_impacts[tech_impacts.ID.str.match(id,case=False)]
	Min_CO2 = impact['Min_CO2(g/kg.DM)'].iloc[0]
	Max_CO2 = impact['Max_CO2(g/kg.DM)'].iloc[0]
	Min_CH4 = impact['Min_CH4(g/kg.DM)'].iloc[0]
	Max_CH4 = impact['Max_CH4(g/kg.DM)'].iloc[0]
	Direct_Labour = impact['Direct_Labour(people/1000t.DM)'].iloc[0]
	
	trans_df2['min_CO2'] = ((trans_df2['tonnes_contributed']*1000)*Min_CO2)/1000000 # min tons of CO2 Emitted per anum
	trans_df2['max_CO2'] = ((trans_df2['tonnes_contributed']*1000)*Max_CO2)/1000000 # max tons of CO2 emitted per anum
	trans_df2['min_CH4'] = ((trans_df2['tonnes_contributed']*1000)*Min_CH4)/1000000 # min tons of CH4 emitted per anum
	trans_df2['max_CH4'] = ((trans_df2['tonnes_contributed']*1000)*Max_CH4)/1000000 # max tons of CH4 emitted per anum
	trans_df2['employed_anum'] = (trans_df2['tonnes_contributed']/1000)* Direct_Labour #number of people employed per anum or job creation potential per anum
	##Export the results
	trans_df2.to_csv(os.path.sep.join(['Processed', id +('_') +(str(trans_df['capacity'].iloc[0]))+('_test.csv')]), index = True)
